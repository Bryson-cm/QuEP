...
# 3D: X, Xi, Y
    ...
    ax3.set_xlabel(r"X ($c/\omega_p$)")
    ax3.set_ylabel(r"$\\xi$ ($c/\omega_p$)")
    ax3.set_zlabel(r"Y ($c/\omega_p$)")
    ax3.set_title(r"Electron Probe Trajectories in $\\xi$")
...
# 3D: X, Z, Y
    ...
    ax4.set_ylabel(r"Z ($c/\omega_p$)")
    ax4.set_zlabel(r"Y ($c/\omega_p$)")
    ax4.set_title(r"Electron Probe Trajectories in $\\xi$")
    for i in range(0, noElec):
